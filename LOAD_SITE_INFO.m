...
PARAMS.Soil.c1 = -1.4e4;
PARAMS.Soil.thetals = 0.2;                                            % value of soil moisture litter at saturation
PARAMS.Soil.thetafc = 0.1;                                          % Value of litter soil moisture at field capacity
PARAMS.Soil.thetatr = 0; %0.18;                                           % Value of soil moisture after which
% rl becomes negligible
PARAMS.Soil.psill = 35.3;                                             % parameter to compute psi litter
PARAMS.Soil.bl = 2.42;                                                % parameter to compute psi litter
PARAMS.Soil.bdl = 42.5;                                               % Bulk density of litter [kg/m3]
...
if SWITCHES.soil3D
    % TOPOGRAPHY
    load './Temps/temp_variable.mat' 'nx' 'ny' 'dx' 'dy' dem;
    HORSTRUC.dem = dem;
    HORSTRUC.nx = nx;
    HORSTRUC.ny = ny;
    HORSTRUC.dx = dx;  % [m] horizontal x direction resolution
    HORSTRUC.dy = dy;  % [m] horizontal y direction resolution 
    

    % NUMERICAL METHOD PARAMETERS
    load './Temps/temp_variable.mat' 'para_numerical';
    if isempty(para_numerical{1,2})
        PARAMS.SOIL3D.iter_param = para_numerical{1,3}; % default 0.55;
    else
        PARAMS.SOIL3D.iter_param = para_numerical{1,2};
    end
    
    if isempty(para_numerical{2,2})
        PARAMS.SOIL3D.max_iter = para_numerical{2,3};
    else
        PARAMS.SOIL3D.max_iter = para_numerical{2,2};
    end
    
    if isempty(para_numerical{3,2})
        PARAMS.SOIL3D.stop_tol = para_numerical{3,3};
    else
        PARAMS.SOIL3D.stop_tol = para_numerical{3,2};
    end
    
    if isempty(para_numerical{4,2})
        PARAMS.SOIL3D.save_interval = para_numerical{4,3};
    else
        PARAMS.SOIL3D.save_interval = para_numerical{4,2};
    end
    
    PARAMS.SOIL3D.BCond = [1, 0, 0, 0, 0, 1];   
    % Boundary condition [top, n, s, e, w, bottom]: 0= no flow, 1=free flow
    % for top and bottom, 0 = no flow for sides, >0 = buffer cells
    if isempty(para_numerical{5,2})
        PARAMS.SOIL3D.BCond(6) = para_numerical{5,3};
    else
        PARAMS.SOIL3D.BCond(6) = para_numerical{5,2};
    end
    
    if isempty(para_numerical{6,2})
        PARAMS.SOIL3D.BCond(2) = para_numerical{6,3};
    else
        PARAMS.SOIL3D.BCond(2) = para_numerical{6,2};
    end
    
    if isempty(para_numerical{7,2})
        PARAMS.SOIL3D.BCond(3) = para_numerical{7,3};
    else
        PARAMS.SOIL3D.BCond(3) = para_numerical{7,2};
    end
    
    if isempty(para_numerical{8,2})
        PARAMS.SOIL3D.BCond(4) = para_numerical{8,3};
    else
        PARAMS.SOIL3D.BCond(4) = para_numerical{8,2};
    end
    
    if isempty(para_numerical{9,2})
        PARAMS.SOIL3D.BCond(5) = para_numerical{9,3};
    else
        PARAMS.SOIL3D.BCond(5) = para_numerical{9,2};
    end
    
    if isempty(para_numerical{10,2})
        par_workers = para_numerical{10,3};
    else
        par_workers = para_numerical{10,2};
    end
    
    % start parallel pool
    if par_workers > 1
        parpool([1,round(par_workers)]);                
    end
    ...
    output_folder = ['./Result' '/Result_3DFlow_Saved_on_' , current_month, '.', current_day, '.', current_year];

    if exist(output_folder, 'dir')
        warningMessage = sprintf('The folder %s already exists! Do you want to overwrite?', output_folder);
        uiwait(warndlg(warningMessage));
    else
        mkdir(output_folder);
    end
    
    PARAMS.SOIL3D.output_folder = output_folder;
    PARAMS.SOIL3D.New_weight = 1;
...
%     air_dry = -15.0;
    ...
    PARAMS.OVERLAND2D.s_min = 1e-7;
    PARAMS.OVERLAND2D.h_min = 1e-5;
end
